...
function gmw_get_search_form_address_field( $gmw, $id=false, $class='') {
    ...
    $output .= '<input type="hidden" name="'.esc_attr( $gmw['url_px'] ).'address[]" id="gmw-address-'.$gmw['ID'].'" autocomplete="off"
    ...
    if ( $gmw['search_form']['locator_icon'] == 'within_address_field' ) {
    	...
    	$output .= '<div class="gmw-locator-btn-wrapper gmw-locator-btn-within-wrapper" style="display:none;">';
...
function gmw_get_search_form_radius_values( $gmw, $class=false ) {
	...
	if ( count( $miles ) > 1 ) {
        ...
        // $output ="<input type=\"hidden\" name=\"$gmw['url_px']}distance\" value=\"1000\" />";
